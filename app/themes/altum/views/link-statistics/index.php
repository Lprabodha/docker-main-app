...
<div class="container">
    ...
    <nav aria-label="breadcrumb">
        <ol class="custom-breadcrumbs small">
            ...
            <li>
                <a href="<?= url('link-update/' . $data->link->link_id) ?>"><?= l('link_update.breadcrumb') ?></a><i class="fa fa-fw fa-angle-right"></i>
            </li>
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        <h1 class="h4 text-truncate mb-0"><?= sprintf(l('link_statistics.header'), $data->link->url) ?></h1>
        ...
        <div class="d-flex align-items-center col-auto p-0">
            ...
            <div>
                <button
                        id="daterangepicker"
                        type="button"
                        class="btn btn-sm btn-outline-secondary cclr"
        ...
        <div class="card borr">
            <div class="card-body">
                <div class="d-flex flex-column align-items-center justify-content-center py-3">
                    <img src="<?= ASSETS_FULL_URL . 'images/no_rows.svg' ?>" class="col-10 col-md-7 col-lg-4 mb-3 img-fluid no-rows-img" alt="<?= l('link_statistics.no_data') ?>" />
    ...
    <?php else: ?>
        ...
        <ul class="account-header-navbar mb-4" role="tablist">
            <li class="nav-item">
                <a class="nav-link <?= $data->type == 'overview' ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=overview&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-list mr-1"></i>
                    <?= l('link_statistics.statistics.overview') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= $data->type == 'entries' ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=entries&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-chart-bar mr-1"></i>
                    <?= l('link_statistics.statistics.entries') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= $data->type == 'country' ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=country&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-globe mr-1"></i>
                    <?= l('link_statistics.statistics.country') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= $data->type == 'city_name' ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=city_name&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-city mr-1"></i>
                    <?= l('link_statistics.statistics.city_name') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= in_array($data->type, ['referrer_host', 'referrer_path']) ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=referrer_host&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-random mr-1"></i>
                    <?= l('link_statistics.statistics.referrer_host') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= $data->type == 'device' ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=device&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-laptop mr-1"></i>
                    <?= l('link_statistics.statistics.device') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= $data->type == 'os' ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=os&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-server mr-1"></i>
                    <?= l('link_statistics.statistics.os') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= $data->type == 'browser' ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=browser&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-window-restore mr-1"></i>
                    <?= l('link_statistics.statistics.browser') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= $data->type == 'language' ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=language&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
                    <i class="fa fa-fw fa-sm fa-language mr-1"></i>
                    <?= l('link_statistics.statistics.language') ?>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link <?= in_array($data->type, ['utm_source', 'utm_medium', 'utm_campaign']) ? 'active' : null ?>" href="<?= url('link-statistics/' . $data->link->link_id . '?type=utm_source&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>">
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/link-statistics/link_statistics_reset_modal.php'), 'modals'); ?>
