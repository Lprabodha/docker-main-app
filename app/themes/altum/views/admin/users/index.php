                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                ...
                                <option selected value="email" <?= $data->filters->search_by == 'email' ? 'selected="selected"' : null ?>><?= l('admin_users.main.email') ?></option>
                            </select>
                        </div>
        ...
        <table class="table table-custom">
            ...
            <tbody>
                <?php foreach ($data->users as $row) : ?>
                    ...
                    <?php
                    $allSubscriptionPlans = db()->where('subscription_id', $row->payment_subscription_id)->where('end_date' > $row->plan_expiration_date)->get('subscriptions');

                    $newPlan =  null;
                    $plan    =  null;
                    if (count($allSubscriptionPlans) > 1) {
                        $newPlan =  end($allSubscriptionPlans);
                        $plan = db()->where('plan_id', $newPlan->plan_id)->getOne('plans', ['name']);
                    }

                    if ($row->plan_id == 'free' && $row->payment_processor == null && (new DateTime($row->plan_expiration_date) > new DateTime())) {
                        $planName = 'Free Trial';
                    } else if ((new DateTime($row->plan_expiration_date) < new DateTime())) {
                        $planName = 'Inactive';
                    } else {
                        $planName = $data->plans[$row->plan_id]->name;
                    }
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex">
                                ...
                                <div class="d-flex flex-column">
                                    ...
                                    <div>
                                        <a class="text-muted" href="<?= url('admin/user-view/' . $row->user_id) ?>"><?= $row->email ?></a>
                                    </div>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex flex-column">
                                <?php if ($user->type != 3) : ?>
                                    ...
                                    <?php if ($row->payment_subscription_id  && ($row->plan_id == 6 || $row->plan_id == 7)) : ?>
                                        <a>The monthly plan is scheduled </a>
                                    <?php elseif ($row->payment_subscription_id != null && isset($plan) && $plan->name) : ?>
                                        <a>The <?= $plan->name ?> plan is scheduled </a>
                                    <?php endif ?>
                        ...
                        <td>
                            <div class="d-flex justify-content-end">
                                <?= include_view(THEME_PATH . 'views/admin/users/admin_user_dropdown_button.php', ['id' => $row->user_id, 'type' => $row->type, 'referral_key' => $row->referral_key, 'resource_name' => $row->name]) ?>
                            </div>
                        </td>
                    </tr>
                <?php endforeach ?>
