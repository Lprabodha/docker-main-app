...
<header class="main-header-wrp plans-header-wrp dpf-header-wrap">
            ...
            <a href="<?= url() ?>" class="animate-logo">
                <div class="logo">
                    <?php if (settings()->main->{'logo_' . \Altum\ThemeStyle::get()} != '') : ?>
                        <img src="<?= ASSETS_FULL_URL . 'images/new-qr-logo.webp' ?>" class="dashboard_logo mt-1 animate-logo" alt="<?= l('global.accessibility.logo_alt') ?>" />
                    <?php else : ?>
                        <?= settings()->main->title ?>
                    <?php endif ?>
                </div>
                ...
                <div class="billing-block dpf dpf-step-block">
                    <nav aria-label="breadcrumb ">
                        <ol class="custom-breadcrumbs small mb-0 ">
                            <li class="<?= $currentUrl == url('plan-rdpf') || $currentUrl == url('plan-rdpf/upgrade') ? 'current' : '' ?> 
                                        <?php if ($currentUrl == url('plan-rdpf') || $currentUrl == url('pay-rdpf/' . $para1) || $currentUrl == url('reactive-plan-dpf/' . $para1) || $currentUrl == url('pay-thank-you-rdpf') || $currentUrl == url('plan-rdpf/upgrade') || $currentUrl == url('change-plan-dpf/' . $para1)) {
                                            echo 'active';
                                        } else {
                                            echo '';
                                        } ?>">
                                <div>
                                    ...
                                    <span class="text d-lg-block d-none"><?= l('plan_dpf.nav_2_plan') ?></span>
                                    <div class="arrow-block"><i class="fa fa-fw fa-angle-right arrow-head"></i></div>
                                    <!--  -->
                                </div>
                            </li>

                            <li aria-current="page" id="menu-payment" class="<?= $currentUrl == url('pay-rdpf/' . $para1) || $currentUrl == url('reactive-plan-dpf/' . $para1) || $currentUrl == url('change-plan-dpf/' . $para1) ? 'current' : '' ?> <?php if ($currentUrl == url('pay-rdpf/' . $para1)  || $currentUrl == url('reactive-plan-dpf/' . $para1) || $currentUrl == url('pay-thank-you-rdpf') || $currentUrl == url('change-plan-dpf/' . $para1)) {
                                                                                                                                                                                                                                                            echo 'active';
                                                                                                                                                                                                                                                        } else {
                                                                                                                                                                                                                                                            echo '';
                                                                                                                                                                                                                                                        } ?>">
                                ...
                                <div>
                                    ...
                                    <span class="text d-lg-block d-none"><?= l('plan_dpf.nav_3_payment') ?></span>
    ...
    <div class="black-friday-full-wrap dpf-black-friday <?= $currentUrl == url('plan-dpf') ? 'd-block dpf-friday-wrap' : 'd-none' ?>">
        <div class="friday-wrap d-flex justify-content-center align-items-center">
            <div class="friday-text-wrap">
                <span class="friday-text"><?= l('plan_dpf.black_friday_text_1') ?><br class="d-block d-sm-none"><span class="color-text"><?= l('plan_dpf.black_friday_text_3') ?></span></span>
            </div>
            <div class="friday-btn-wrap ps-4">
                <button type="button" class="btn friday-btn"><?= l('plan_dpf.black_friday_btn') ?></button>
            </div>
        </div>
    </div>
</header>
