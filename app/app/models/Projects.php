...
class Projects extends Model {
    ...
    public function get_projects_by_user_id($user_id) {
        ...
        if(is_null($cache_instance->get())) {
            ...
            $subQry = "(SELECT COUNT(`project_id`) FROM `qr_codes` WHERE  `status` = '1' AND `project_id` = `projects`.`project_id`)  AS total_project";
            $projects_result = database()->query("SELECT *, $subQry  FROM `projects` WHERE `user_id` = {$user_id}");
