...
class App
{
    ...
    public function __construct()
    {
        ...
        AnalyticsDatabase\AnalyticsDatabase::initialize();
        ...
        if (APP_CONFIG == 'local') {
            ...
            if ($lang_code) {
                ...
                if ($browser_language) {
                    if (\Altum\Language::$name != $browser_language) {
                        \Altum\Language::set_default_by_name($browser_language);
                    }
                } else {
        ...
        if (Authentication::check()) {
            ...
            if (!$user->plan || ($user->plan && ((new \DateTime()) > (new \DateTime($user->plan_expiration_date)) && $user->plan_id != 'free') || !$user->plan->status)) {
                ...
                if (!in_array(Router::$controller_key, ['index', 'affiliate', 'contact', 'page', 'pages', 'plan', 'pay', 'pay-billing', 'pay-thank-you', 'account', 'account-plan', 'account-payments', 'invoice', 'account-logs', 'account-delete', 'referrals', 'account-api', 'account-redeem-code', 'logout', 'register', 'teams-system', 'teams-member', 'teams-members','billing']) && Router::$path != 'admin') {
                    redirect('plan/new');
                }
            }
        ...
        /* Give the guest settings for his plan */ else {

            Authentication::$user = new \StdClass();
            Authentication::$user->user_id = null;
            Authentication::$user->timezone = Date::$default_timezone;
        ...
        // temapory user redirect home page
        if (Authentication::is_temp_user()) {
            if (in_array(Router::$controller_key, ['qr-codes', 'dashboard', 'analytics']) && Router::$path != 'admin') {
                Authentication::logout();
                return false;
            }
        }
        ...
        if (isset(settings()->main->auto_language_detection_is_enabled) && settings()->main->auto_language_detection_is_enabled && Router::$controller_settings['no_browser_language_detection'] == false && !Router::$language_code && !Authentication::check() && $browser_language_code && Language::$default_code != $browser_language_code && array_search($browser_language_code, Language::$active_languages)) {
        ...
        AnalyticsDatabase\AnalyticsDatabase::close();
    }
}