...
class AdminQrCodes extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_GET['search_by']) && $_GET['search_by'] == "user_id") {
            $user = db()->where('email', $_GET['search'])->getOne('users');
            if ($user) {
                $_GET['search'] = $user->user_id;
            }
        }

        /* Prepare the filtering system */
        $filters = (new \Altum\Filters(['user_id', 'project_id', 'type', 'email', 'name', 'uid'], ['name', 'user_id', 'uId'], ['name', 'datetime']));
        ...
        $qr_codes_result = database()->query("
            SELECT
                `qr_codes`.`qr_code_id`,`qr_codes`.`user_id`,`qr_codes`.`settings`,`qr_codes`.`uId`,`qr_codes`.`status`,`qr_codes`.`project_id`,`qr_codes`.`type`,`qr_codes`.`name`,`qr_codes`.`last_datetime`,`qr_codes`.`datetime`, `users`.`name` AS `user_name`, `users`.`email` AS `user_email`
            FROM
                `qr_codes`
            LEFT JOIN
            ...
            WHERE
                1 = 1
                AND (`qr_codes`.`user_id` IS NOT NULL AND `users`.`type` != 2)
    ...
    public function transfer() {
        ...
        if(!$current_user = db()->where('user_id', $qr_code->user_id)->getOne('users', ['user_id', 'email'])) {
            Alerts::add_error(l('admin.transfer_modal.user_not_found'));
            redirect('admin/qr-codes');
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            $qrJsonData['userIdAjax'] = $new_user->user_id;
            ...
            /* Set a nice success message */
            Alerts::add_success(sprintf(l('admin.transfer_modal.success_message'), '<strong>' . input_clean($qr_code->name) . '</strong>', '<strong>' . input_clean($current_user->email) . '</strong>', '<strong>' . input_clean($new_user->email) . '</strong>'));
