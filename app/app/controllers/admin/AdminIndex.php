...
class AdminIndex extends Controller {
    ...
    public function index() {
        ...
        if(in_array(settings()->license->type, ['Extended License', 'extended'])) {
            $payments = db()->where('status', 1)->where('is_refund', 0)->getValue('payments', 'count(`id`)');
            $payments_total_amount = db()->where('status', 1)->where('is_refund', 0)->getValue('payments', 'sum(`total_amount`)');
        } else {
