...
class Invoice extends Controller {
    ...
    public function index() {
        ...
        $payment->previous_plan = json_decode($payment->previous_plan);
        $payment->credit_amount = json_decode($payment->credit_amount);   
        $payment->new_plan_id = json_decode($payment->new_plan_id);        
        ...
        // $currentTime =  (new \DateTime($payment->datetime))->setTimezone(new \DateTimeZone(\Altum\Date::$timezone))->format('M j, Y H:i:s');

        $paymentDateTime = (new \DateTime($payment->datetime))->setTimezone(new \DateTimeZone(\Altum\Date::$timezone));
        // Get the numeric month value
        $month = $paymentDateTime->format('n');
        $currentTime = l("global.date.short_months.{$month}") . ' ' . $paymentDateTime->format('j, Y H:i:s');
        $userTaxDetails = db()->where('user_id', $this->user->user_id)->getOne('taxes');              
               
        /* Prepare the View */
        $data = [
            'user' => $this->user,
            'payment' => $payment,
            'user_tax_data' => $userTaxDetails,
