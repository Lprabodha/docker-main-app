    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            if(db()->where('email', $_POST['email'])->has('users') && $_POST['email'] !== $this->user->email) {
                Alerts::add_field_error('email', l('global.account.error_message.email_exists'));
            }

            if(mb_strlen($_POST['name']) < 1 || mb_strlen($_POST['name']) > 64) {
                Alerts::add_field_error('name', l('global.account.error_message.name_length'));
            }

            if(!empty($_POST['old_password']) && !empty($_POST['new_password'])) {
                if(!password_verify($_POST['old_password'], $this->user->password)) {
                    Alerts::add_field_error('old_password', l('global.account.error_message.invalid_current_password'));
                }
            ...
            if($_POST['twofa_is_enabled'] && $_POST['twofa_token']) {
                ...
                if(!$twofa_check) {
                    Alerts::add_field_error('twofa_token', l('global.account.error_message.twofa_check'));
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                Alerts::add_success(l('global.account.success_message.account_updated'));
                ...
                if($_POST['email'] != $this->user->email) {
                    ...
                    if(settings()->users->email_confirmation) {
                        ...
                        Alerts::add_info(l('global.account.info_message.user_pending_email'));
                ...
                if(!empty($_POST['old_password']) && !empty($_POST['new_password'])) {
                    ...
                    /* Set a nice success message */
                    Alerts::add_success(l('global.account.success_message.password_updated'));
        ...
        $taxData = db()->where('user_id', $this->user->user_id)->getOne('taxes');
