...
class AccountRedeemCode extends Controller {
    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            $code = $_POST['code'] = trim(Database::clean_string($_POST['code']));
            ...
            if(!$code = database()->query("SELECT * FROM `codes` WHERE `code` = '{$code}' AND `type` = 'redeemable' AND `redeemed` < `quantity`")->fetch_object()) {
                Alerts::add_field_error('code', l('global.account.account_redeem_code.error_message.code_invalid')); redirect('account-redeem-code');
            }
            ...
            if(!in_array($code->code_id, $plan->codes_ids)) {
                Alerts::add_field_error('code', l('global.account.account_redeem_code.error_message.code_invalid'));
            }

            if(db()->where('user_id', $this->user->user_id)->where('code_id', $code->code_id)->has('redeemed_codes')) {
                Alerts::add_field_error('code', l('global.account.account_redeem_code.error_message.code_used'));
            }
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                Alerts::add_success(sprintf(l('global.account.account_redeem_code.success_message'), $code->days, $plan->name));
        ...
        $view = new \Altum\Views\View('account-redeem-code/index', (array) $this);
